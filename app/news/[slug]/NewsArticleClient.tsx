...
function NewsArticleContent({ initialArticle }: { initialArticle: NewsArticle | null }) {
  ...
  // Use server-fetched article as initial state, with fallback to static data
  const [article, setArticle] = useState<NewsArticle | null>(
    initialArticle ?? newsArticles.find((a) => a.slug === slug) ?? null
  );
  const [loading, setLoading] = useState(false);

  // Fetch fresh article from API (with automatic fallback to static data)
  useEffect(() => {
